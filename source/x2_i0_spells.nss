...
void CheckAndApplyThunderingRage(int nRounds)
{
    ...
    if (GetHasFeat(988, OBJECT_SELF))
    {
        ...
        if(!IPGetIsMeleeWeapon(oWeapon)){nRounds=0;}
        if (GetIsObjectValid(oWeapon) && nRounds>0)
        {
           IPSafeAddItemProperty(oWeapon, ItemPropertyMassiveCritical(IP_CONST_DAMAGEBONUS_2d12), RoundsToSeconds(nRounds), X2_IP_ADDPROP_POLICY_REPLACE_EXISTING,FALSE,TRUE);
           ...
           IPSafeAddItemProperty(oWeapon, ItemPropertyOnHitProps(IP_CONST_ONHIT_DEAFNESS,IP_CONST_ONHIT_SAVEDC_26,IP_CONST_ONHIT_DURATION_25_PERCENT_3_ROUNDS), RoundsToSeconds(nRounds), X2_IP_ADDPROP_POLICY_REPLACE_EXISTING,FALSE,TRUE);
        }
        oWeapon =  GetItemInSlot(INVENTORY_SLOT_LEFTHAND);
        if (GetIsObjectValid(oWeapon) && nRounds>0)
        {
           IPSafeAddItemProperty(oWeapon, ItemPropertyMassiveCritical(IP_CONST_DAMAGEBONUS_2d12), RoundsToSeconds(nRounds), X2_IP_ADDPROP_POLICY_KEEP_EXISTING,TRUE,TRUE);
           IPSafeAddItemProperty(oWeapon,ItemPropertyVisualEffect(ITEM_VISUAL_SONIC), RoundsToSeconds(nRounds), X2_IP_ADDPROP_POLICY_REPLACE_EXISTING,FALSE,TRUE);
           IPSafeAddItemProperty(oWeapon, ItemPropertyOnHitProps(IP_CONST_ONHIT_DEAFNESS,IP_CONST_ONHIT_SAVEDC_26,IP_CONST_ONHIT_DURATION_25_PERCENT_3_ROUNDS), RoundsToSeconds(nRounds), X2_IP_ADDPROP_POLICY_REPLACE_EXISTING,FALSE,TRUE);
        }
        effect eDamageBonus = EffectDamageIncrease(DAMAGE_BONUS_2d4,DAMAGE_TYPE_ELECTRICAL);
        eDamageBonus = ExtraordinaryEffect(eDamageBonus);
        if(!GetHasEffect(EFFECT_TYPE_POLYMORPH,OBJECT_SELF))
        {
            if(nRounds>0 && nLevel>25)
            {
            ApplyEffectToObject(DURATION_TYPE_TEMPORARY,eDamageBonus,OBJECT_SELF,RoundsToSeconds(nRounds));
            }
        }
     }
}
...
void DoMindBlast(int nDC, int nDuration, float fRange)
{
    ...
    while(GetIsObjectValid(oTarget))
    {
        ...
        if(spellsIsTarget(oTarget,SPELL_TARGET_SELECTIVEHOSTILE,OBJECT_SELF) && oTarget != OBJECT_SELF && !bImmune )
        {
            ...
            if (GetHasSpellEffect(GetSpellId(),oTarget))
            {
                ...
                {
                    nDamage = d10(GetLevelByClass(CLASS_TYPE_SHIFTER,OBJECT_SELF));
                }
                else
                {
...
void EngulfAndDamage(object oTarget, object oSource)
{
  ...
  if (ReflexSave(oTarget, 22 + GetHitDice(oSource), SAVING_THROW_TYPE_NONE,oSource) == 0)
  {

      FloatingTextStrRefOnCreature(84610,oTarget);
      int nDamage = d2(GetHitDice(oSource));
      nDamage = nDamage/3*2;
