...
void Spawn()
{
    ...
    if(nPCCount > 0)
        SetLocalInt(OBJECT_SELF, "MY_PC_COUNT", nPCCount); //J. Persinne; store this value for use in other functions.
    else
        SetLocalInt(OBJECT_SELF, "MY_PC_COUNT", -1); //J; Persinne; value of -1 used for areas which house NESS creatures in which no PCs exist.
    ...
    else if(nPCCount == 0) //J. Persinne; process no more than each five minutes if no one is in the area.
    {
        int nHBCount = GetLocalInt(OBJECT_SELF, "HB_COUNT");
        nHBCount++;
        SetLocalInt(OBJECT_SELF, "HB_COUNT", nHBCount);
        if(nHBCount < 51 && nHBCount != 1)
            return;
        else if(nHBCount >= 51)
    ...
    for (nNth = 1; nNth <= nSpawns; nNth++)
    {
        ...
        if (nPCCount > 0 && (nCurrentProcessTick > nProcessFrequency || nProcessFrequency == 1))
        {
            ...
            if (nSpawnNumSaveStates > 0 || nSpawnNumSaveCampStates > 0)
            {
                ...
                //nForcedRespawns = TRUE;
            }
        }

        if (nCurrentProcessTick == 0 && GetLocalInt(oSpawn, "OFFSET_DONE") != TRUE)
        {
            //  First time in. Always process the first time
            //nProcessSpawn = TRUE;
            SetLocalInt(oSpawn, "CurrentProcessTick", 2-nProcessOffset);
            SetLocalInt(oSpawn, "OFFSET_DONE", TRUE);
            //debug("Tick 1");
            //debug("+");
        }
        ...
        if(nPCCount == 0) //J. Persinne; always process when no PCs are in the area so that we handle despawning properly.
        {
            nProcessSpawn = TRUE;
        }
   ...
   int nTimeNow, int nWaypoint)
{
    ...
    for (nChildSlot = 1; nChildSlot <= nSpawnNumber; nChildSlot++)
    {
        ...
        else
        {
            if (nPlaceable == FALSE && nSpawnCamp == FALSE && nSpawnItem == FALSE)
            {
                ...
                object oMaster = GetMaster(oCreature);              //J. Persinne; don't process behaviors on a PC-controlled creature.
        ...
        if (nSpawnChild == TRUE)
        {
            ...
            {
                if (CountPCsInRadius(lSpawn, fDespawnTrigger, TRUE) == FALSE)
                {
    ...
    if (nEmptyChildSlots > 0)
    {
        ...
        if (fSpawnTrigger > 0.0)
        {
            ...
            if (CountPCsInRadius(lSpawn, fSpawnTrigger, TRUE) == FALSE)
            {
                //debug("no one close");
                nSpawnBlock = TRUE;
            }
