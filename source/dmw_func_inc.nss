...
{
   WriteTimestampedLogEntry ("DM Saving all characters...");
   ExportAllCharacters ();
   WriteTimestampedLogEntry ("DM Saving Complete.");
}
...
//      if(GetItemHasItemProperty(oItem, ITEM_PROPERTY_ENHANCED_CONTAINER_BONUS_SLOTS)) { sProperties = sProperties + "Enhanced Container Bonus Slots\n"; }
      ...
      if(GetItemHasItemProperty(oItem, IP_CONST_ONHIT_VORPAL)) { sProperties = sProperties + "Vorpal\n"; }
      if(GetItemHasItemProperty(oItem, IP_CONST_ONHIT_WOUNDING)) { sProperties = sProperties + "Wounding\n"; }
...
void dmwand_KickPC()
{
   ...
   AssignCommand( oMySpeaker, ApplyEffectAtLocation ( DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_NONE), lMyLoc));
   AssignCommand ( oMySpeaker, PlaySound ("bf_tiny"));
   object oScorch = CreateObject ( OBJECT_TYPE_PLACEABLE, "booted", lMyLoc, FALSE);
   ...
   for(nCount = 0; nCount < 5; nCount++)
   {
      ...
      AssignCommand( oMySpeaker, ApplyEffectAtLocation ( DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_NONE), lNewLoc));
   }
...
void dmwand_ShowAllAttribs()
{
   ...
   string sPlayerName = GetPCPlayerName(oMyTarget);
   string sCDkey = GetPCPublicCDKey(oMyTarget);
   string sReport = "\n-------------------------------------------" +
...
void dmwand_Zombie()
{
   effect ePenguin = EffectPolymorph(POLYMORPH_TYPE_ZOMBIE);

   AssignCommand(oMyTarget, ApplyEffectToObject(DURATION_TYPE_PERMANENT, ePenguin, oMyTarget));
   SetLocalInt(oMyTarget, "toaded", 1);
}

void dmwand_Werewolf()
{
   effect ePenguin = EffectPolymorph(POLYMORPH_TYPE_WEREWOLF);
