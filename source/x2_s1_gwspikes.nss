    ...
    int nVIS = VFX_IMP_MAGBLUE, int nDAMAGETYPE = DAMAGE_TYPE_MAGICAL, int nONEHIT = FALSE)
{
    ...
    oTarget = GetFirstObjectInShape(SHAPE_SPHERE, RADIUS_SIZE_GARGANTUAN, lTarget, TRUE, OBJECT_TYPE_CREATURE);
    //Cycle through the targets within the spell shape until an invalid object is captured.
    while (GetIsObjectValid(oTarget) && nCnt <= nEnemies && LineOfSightObject(OBJECT_SELF,oTarget)==TRUE)
    {
        ...
        if (spellsIsTarget(oTarget,SPELL_TARGET_SELECTIVEHOSTILE,OBJECT_SELF) && (oTarget != OBJECT_SELF))
        {
                ...
                for (i=1; i <= nExtraMissiles + nRemainder; i++)
                {
                    ...
                    if (nResult > 0)
                    {
                        ...
                        //Set damage effect
                        effect eDam = EffectDamage(nDam, nDAMAGETYPE,DAMAGE_POWER_PLUS_TWO);
...
void main()
{
    ...
    if (nCasterLevel == 0)
    {
        nCasterLevel = MothGetCasterLevel(OBJECT_SELF);
        if (nCasterLevel == 0)
        {
