...
int GetAverageClass(object oEnemy);
...
void DetermineCombatRound(object oIntruder = OBJECT_INVALID, int nAI_Difficulty = 10)
{
       ...
       (GetIsObjectValid(oIntruder) && GetIsObjectValid(GetMaster())))
    {
        ...
        if(nClass == CLASS_TYPE_WIZARD || nClass == CLASS_TYPE_SORCERER || nClass == CLASS_TYPE_BARD)
        {
            ...
            // Added by Jugalator - attempt to cast from hand-picked list of "important" spells
            ...
            nClass == CLASS_TYPE_BARBARIAN)
        {
            ...
            if (nClass == CLASS_TYPE_RANGER) {
                if (TalentImportantRangerSpells()) { return; } // streamlined divine spells for ranger
            } else if (nClass == CLASS_TYPE_PALADIN) {
                if (TalentImportantPaladinSpells()) { return; } // streamlined divine spells for paladin
            }
...
void EquipAppropriateWeapons(object oTarget)
{
    ...
    else
    {
        ...
        {
            if(GetDistanceToObject(oTarget) > 15.0 && GetWeaponRanged(oWeapon)==TRUE)
            {
                ActionEquipMostDamagingRanged(oTarget);
            }
            else
...
//    object oPC = GetNearestCreature(CREATURE_TYPE_PLAYER_CHAR, PLAYER_CHAR_IS_PC, OBJECT_SELF, 1, CREATURE_TYPE_REPUTATION, REPUTATION_TYPE_ENEMY);
    ...
    int iPartyHD = GetAverageClass(oPC);
    ...
    // Hm.. This should only happen if a casting continues over the next turn,
    // which *has* happened occasionally (and shouldn't), but I'm not sure what to do.
    // For now, don't perform any actions that turn if it happens...
    // (instead of clearing the action)
    if (GetCurrentAction() == ACTION_CASTSPELL) {
        return TRUE; // skip remaining talents
    }
    ...
    //Time Stop, 100% chance
    // does GetHasSpellEffect properly recognize the Time Stop "effect"?
    if (!GetHasSpellEffect(SPELL_TIME_STOP)) {
        if (Jug_AtObject(SPELL_TIME_STOP, OBJECT_SELF, iMod, 19)) return TRUE;
    }

    //Circle of Death, 80% chance
    //CONDITION: Average enemy party hd less than 10
    if (iPartyHD < 10) {
        if (Jug_AtLocation(SPELL_CIRCLE_OF_DEATH, oPC, iMod, 16, 80)) return TRUE;
    }

    //Power Word: Stun, 60% chance
    //CONDITION: Targeted (and closest) enemy HD less than 14
    if (GetHitDice(oPC) < 14) {
        if (Jug_AtObject(SPELL_POWER_WORD_STUN, oPC, iMod, 17, 60)) return TRUE;
    }

    //Physical Damage Protections, 100% chance
    ...
    //Visibility Protections, 100% chance
    ...
    //Shapechange, 20% chance
    if (!GetHasSpellEffect(SPELL_SHAPECHANGE)) {
        if (Jug_AtObject(SPELL_SHAPECHANGE, OBJECT_SELF, iMod, 19, 20)) return TRUE;
    }

    //Protection from Spells, 30% chance
    ...
    {
        if (Jug_AtObject(SPELL_PROTECTION_FROM_SPELLS, OBJECT_SELF, iMod, 17, 30)) return TRUE;
    }

    //Mantle Protections, 60% chance
    //CONDITION: Enemies less than 8 levels below me (powerful enemies)
    if (!(GetHasSpellEffect(SPELL_GREATER_SPELL_MANTLE) || GetHasSpellEffect(SPELL_SPELL_MANTLE) ||
        GetHasSpellEffect(SPELL_LESSER_SPELL_MANTLE)) && (iPartyHD - iMyHD) >= -8 && d100() < 60)
    {
    ...
    //Power Word: Kill, 80% chance
    //CONDITION: Targeted enemy HD less than 10
    if (GetHitDice(oPC) < 10) {
        if (Jug_AtObject(SPELL_POWER_WORD_KILL, oPC, iMod, 19, 80)) return TRUE;
    }

    //Meteor Swarm, 80% chance
    //CONDITION: Closest enemy 5 ft. (1.5 meters) from me
    if (GetDistanceToObject(oPC) > 1.5f) {
        if (Jug_AtLocation(SPELL_METEOR_SWARM, oPC, iMod, 19, 80)) return TRUE;
    }

    //Horrid Wilting, 80% chance
    if (Jug_AtLocation(SPELL_HORRID_WILTING, oPC, iMod, 18, 80)) return TRUE;
    ...
    // Energy Drain, 40% chance
    if (Jug_AtObject(SPELL_ENERGY_DRAIN, oPC, iMod, 19, 40)) return TRUE;

    //Wail of the Banshee, 70% chance
    if (Jug_AtLocation(SPELL_WAIL_OF_THE_BANSHEE, oPC, iMod, 19, 70)) return TRUE;

    //Weird, 70% chance
    if (Jug_AtObject(SPELL_WEIRD, oPC, iMod, 19, 70)) return TRUE;
    ...
    // Finger of Death, 70% chance
    if (Jug_AtObject(SPELL_FINGER_OF_DEATH, oPC, iMod, 17, 70)) return TRUE;

    // Globes, 60% chance
    if (!(GetHasSpellEffect(SPELL_GLOBE_OF_INVULNERABILITY) ||
        GetHasSpellEffect(SPELL_MINOR_GLOBE_OF_INVULNERABILITY)) && d10() <= 6)
    {
    ...
    {
        if (Jug_AtLocation(SPELL_GATE, oPC, iMod, 19, 90)) return TRUE;
    }

    //Protection from Evil / Magic Circle Against Evil, 100% chance
    ...
    //Elemental Protections, 80% chance
    if (!(GetHasSpellEffect(SPELL_ENERGY_BUFFER) || GetHasSpellEffect(SPELL_PROTECTION_FROM_ELEMENTS)))
    {
        if (Jug_AtObject(SPELL_ENERGY_BUFFER, OBJECT_SELF, iMod, 15, 80)) return TRUE;
        if (Jug_AtObject(SPELL_PROTECTION_FROM_ELEMENTS, OBJECT_SELF, iMod, 13, 80)) return TRUE;
    }

    //Misc Protections, 50% chance
    if (!GetHasSpellEffect(SPELL_ELEMENTAL_SHIELD)) {
        if (Jug_AtObject(SPELL_ELEMENTAL_SHIELD, OBJECT_SELF, iMod, 14, 50)) return TRUE;
    }

    //Bard War Cry, 80% chance
    if (Jug_AtObject(SPELL_WAR_CRY, OBJECT_SELF, iMod, 14, 80)) return TRUE;
...
// Remember Lesser Planar Binding has special effect if targetting Outsider
// Not sure what would happen if targeting enemy location so I'm leaving it for now
//      if (Jug_AtLocation(SPELL_LESSER_PLANAR_BINDING, oPC, iMod, 15)) return TRUE;
    ...
    // add range check to those to avoid killing self?
    if (Jug_AtLocation(SPELL_DELAYED_BLAST_FIREBALL, oPC, iMod, 17, 30)) return TRUE;
    if (Jug_AtLocation(SPELL_FIREBALL, oPC, iMod, 13, 30)) return TRUE;
...
//  if (Jug_AtLocation(SPELL_MIND_FOG, oPC, iMod, 16, 30)) return TRUE;
    ...
    // -------------------------------------------------------------------------
    // MISSING SPELLS (although they *are* all cast by default AI)
    // -------------------------------------------------------------------------
    // NOTE: Negative Energy Blast not supported - need to check for undeads :)
    // NOTE: Find Traps not supported
    // NOTE: Clairaudience & Clairvoyance not supported
    // NOTE: Clarity not supported - might be an idea to do, but need special checks
    // NOTE: Dispel Magic not supported - leaving for default AI
    // NOTE: Invisibility Sphere not supported
    // NOTE: Vampiric Touch not supported - I don't wanna force enter melee range :/
    // NOTE: Charm Monster not supported - uncertain about its efficiency
    // NOTE: Shadow Conjuration not supported yet
    // NOTE: Dismissal not supported yet - would like to add this later!
    // NOTE: Greater Shadow Conjuration not supported yet - useful enough to have it here?
    // NOTE: Lesser Mind Blank not supported - useful enough to have it here?
    // NOTE: Lesser Planar Binding not supported - see summoning section
    // NOTE: Mind Fog not supported yet - see above
    // NOTE: Legend Lore not supported
    // NOTE: Planar Binding not supported - same reason as for Lesser version
    // NOTE: Tenser's Transformation not supported - might be fun to do though :)
    // NOTE: True Seeing not supported yet - need to check for invisibles
    // NOTE: Control Undead not supported yet - need to check for undeads
    // NOTE: Mordenkainen's Disjunction not supported yet - need to check for target enchantements somehow
    // -------------------------------------------------------------------------
...
//    object oPC = GetNearestCreature(CREATURE_TYPE_PLAYER_CHAR, PLAYER_CHAR_IS_PC, OBJECT_SELF, 1, CREATURE_TYPE_REPUTATION, REPUTATION_TYPE_ENEMY);
    ...
    // For now, don't perform any actions that turn if it happens...
    if (GetCurrentAction() == ACTION_CASTSPELL) {
        return FALSE; // skip remaining talents
    }
    ...
    //CONDITION: Affected by Slow or Web. Otherwise 12.5% chance to cast each combat round
    if ((GetHasSpellEffect(SPELL_SLOW) || GetHasSpellEffect(SPELL_WEB)) &&
        !GetHasSpellEffect(SPELL_FREEDOM_OF_MOVEMENT))
    {
        if (Jug_AtObject(SPELL_FREEDOM_OF_MOVEMENT, oPC, iMod, 14)) return TRUE;
    }

    // ####
    // Check for allies (and if they're hurt) here and use Healing Circle!
    // ####
    ...
    // Heal (used as Harm for undead)
    // CONDITION: Undead at 6+ hd
    if (GetRacialType(oPC) == RACIAL_TYPE_UNDEAD && iPartyHD > 6)
    {
    ...
    if (!isDruid || isMonster) {
        ...
        // ###
        // Check for invis and cast True Seeing!
        // ###
    }
    if (isDruid || isMonster) {
    ...
    // ### ADD CHECK FOR ENEMY SUMMONS!
    if (Jug_AtObject(SPELL_DISMISSAL, OBJECT_SELF, iMod, 14)) return TRUE;
...
//    object oPC = GetNearestCreature(CREATURE_TYPE_PLAYER_CHAR, PLAYER_CHAR_IS_PC, OBJECT_SELF, 1, CREATURE_TYPE_REPUTATION, REPUTATION_TYPE_ENEMY);
    ...
    if (GetCurrentAction() == ACTION_CASTSPELL) {        return FALSE;
    }
    ...
    //Freedom of Movement - Negates effects of paralysis, Hold, Slow, or Web spells
    //CONDITION: Affected by Slow or Web. Otherwise 12.5% chance to cast just
    // for the heck of it :) (as a precaution)
    if ((GetHasSpellEffect(SPELL_SLOW) || GetHasSpellEffect(SPELL_WEB) || d6() == 1) &&
        !GetHasSpellEffect(SPELL_FREEDOM_OF_MOVEMENT))
    {
    ...
    // MISSING RANGER SPELLS IN IMPROVED AI SCRIPT
    //Invisibility Purge - Dispels Invisibility on creatures and items
    //Neutralize Poison - Stops the effects of Poison
    //Remove Disease - Cancels the effects of disease
    //Ultravision - Grants Ultravision to target for 1hr/level
    //*** RANGE CHECK TO NOT ENTAGLE SELF *** Entangle - Targets suffer -2 attack penalty and -4 Dex
    //*** RANGE CHECK TO NOT GREASE SELF *** Grease - Creatures in area must save or fall down
    //*** NEED ANIMAL CHECK *** Hold Animal - Immobilizes 1 animal for 1 round/level
}
...
//    object oPC = GetNearestCreature(CREATURE_TYPE_PLAYER_CHAR, PLAYER_CHAR_IS_PC, OBJECT_SELF, 1, CREATURE_TYPE_REPUTATION, REPUTATION_TYPE_ENEMY);
    ...
    //TODO: Also check allies to cast on those?
    ...
    //FIXME: Check for paralyzed allies!
//    if (GetHasSpell(SPELL_REMOVE_PARALYSIS) && iMod >= 12 && d6() == 1) {
//    }
    ...
    if (!GetHasSpellEffect(SPELL_PRAYER)) {
        if (Jug_AtLocation(SPELL_PRAYER, OBJECT_SELF, iMod, 13)) return TRUE;
    }
    ...
    //Bull's Grace - Target gains 1d4+1 Str for 1hr/level
    ...
    // MISSING PALADIN SPELLS IN IMPROVED AI SCRIPT
    //*** NEED ALLY PARALYSIS CHECK *** Remove Paralysis - Frees allies from paralysis, Hold, or Slow spells
    //*** NEED HARMFUL MAGIC CHECK (on allies?) *** Dispel Magic
    //Eagle's Splendor - not useful in combat
    //Neutralize Poison - not useful in combat
    //Level 1 spells - will never be supported here, let default AI take care of those
}

int GetAverageClass(object oEnemy)
{
    struct sEnemies enemies = DetermineEnemies();
    if(!enemies.TOTAL) enemies.TOTAL=1;
...
int Jug_AtLocation(int nSpellID, object oTarget, int iModifier, int iRequirement, int iChance = 100)
{
    ...
    {
        //ClearAllActions at start of respective TalentImportant* function
