...
void main()
{
        ...
        SetLocalInt(oMod,"MOTHCLEGATEDOWN",1);
        DelayCommand(fDelay,DeleteLocalInt(oMod,"MOTHCLEGATEDOWN"));
        ...
        WriteTimestampedLogEntry ("Outer Gate Destroyed by:" + sPlayerName +", Handle:" + GetName(oPC)  +", CD KEY:" + sCDK);
        object oPC2 = GetFirstPC();
        while (GetIsObjectValid(oPC2))
        {
            AssignCommand (oPC2, PlaySound("as_an_wolfhowl1"));
            SendMessageToPC(oPC2, "<c  þ>[Faction] Axfells have destroyed the Cleaven Outer Gate!</c>");
            oPC2 = GetNextPC();
        }
}
