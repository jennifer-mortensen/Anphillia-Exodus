...
void RunHandImpact3(object oTarget, object oCaster)
{
    int nBonus2 = MothGetCasterLevel(oCaster)/2;
    if (GZGetDelayedSpellEffectsExpired(461,oTarget,oCaster))
    {
        return;
    }
    int nDam = d12(4);
    effect eDam = EffectDamage(nDam, DAMAGE_TYPE_BLUDGEONING,DAMAGE_POWER_PLUS_FOUR);
    effect eVis = EffectVisualEffect(VFX_IMP_ACID_L);
    ApplyEffectToObject(DURATION_TYPE_INSTANT, eDam, oTarget);
    ApplyEffectToObject(DURATION_TYPE_INSTANT, eVis, oTarget);
    DelayCommand(9.0f,RunHandImpact3(oTarget,oCaster));
}
void main()
{
    ...
    if(!GetIsReactionTypeFriendly(oTarget))
    {
        ...
        if(!MyResistSpell(OBJECT_SELF, oTarget))
        {
            ...
            RunHandImpact3(oTarget, OBJECT_SELF);
