...
//    ZEP_CR_DONE_NOGOLD
//    ZEP_CR_DONE_NODYE
void ZEP_AttemptCraft2(object oPC);
...
int ZEP_GetModificationCost2(object oItem, object oNew, int nNPC=FALSE);

// Gets the DC to modify oItem into oNew
// Calls the default bioware routines for this, edit this function to override
int ZEP_GetModificationDC2(object oItem, object oNew, int nNPC=FALSE);
...
void ZEP_AttemptCraft2(object oPC) {
    ...
    int nCost = ZEP_GetModificationCost2(oBackup, oItem, nNPC);
    int nDC   = ZEP_GetModificationDC2(oBackup, oItem, nNPC);
    int nSkill;
...
int ZEP_GetModificationCost2(object oItem, object oNew, int nNPC=FALSE) {
    ...
    //Moth override all caluculations to 0
    nCost=0;
    return nCost;
}
...
void ZEP_RemakeItem(object oPC, int nMode) {
    ...
    if (GetIsObjectValid(oNew)) {
        ...
        int nNPC = GetIsObjectValid(GetLocalObject(oPC, "ZEP_CR_NPC"));
        object oBackup = GetLocalObject(oPC, "ZEP_CR_BACKUP");
        nCost = ZEP_GetModificationCost2(oBackup, oNew, nNPC);
        nDC   = ZEP_GetModificationDC2(oBackup, oNew, nNPC);
