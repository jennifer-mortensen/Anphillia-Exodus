...
int iBaseType;
    ...
    {
    oMyWeapon = GetItemInSlot(INVENTORY_SLOT_CHEST, oTarget);
    if(MothGetIsClothing(oMyWeapon)){nDuration=0;}
    }
...
DelayCommand(1.0,FloatingTextStringOnCreature("<cڥ#>*Armor Enhancement Applied!*</c>",oPC));
...
DelayCommand(1.0,AddItemProperty(DURATION_TYPE_TEMPORARY,ItemPropertyDamageResistance(IP_CONST_DAMAGETYPE_PIERCING,IP_CONST_DAMAGERESIST_5),oMyWeapon,RoundsToSeconds(nDuration)));
}
return;
}
else
