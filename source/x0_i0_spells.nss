...
int MothGetReflexAdjustedDamageTrap(int nDamage, object oTarget, int nDC, int nSaveType=SAVING_THROW_TYPE_NONE, object oSaveVersus=OBJECT_SELF)
{
...
//* fires a storm of nCap missiles at targets in area
void DoMissileStorm2(int nD6Dice, int nCap, int nSpell, int nMIRV = VFX_IMP_MIRV, int nVIS = VFX_IMP_MAGBLUE, int nDAMAGETYPE = DAMAGE_TYPE_MAGICAL, int nONEHIT = FALSE, int nReflexSave = FALSE);
...
void DoTrapSpike(int nDamage)
{
    object oTarget = GetEnteringObject();
        int nRealDamage = MothGetReflexAdjustedDamageTrap(nDamage, oTarget, 15, SAVING_THROW_TYPE_TRAP, OBJECT_SELF);
        if (nDamage > 0)
        {
   ...
   // SpawnScriptDebugger();
    ...
    if (spellsIsTarget(oTarget, SPELL_TARGET_SELECTIVEHOSTILE, GetAreaOfEffectCreator()))
    {
        ...
        if(!MyResistSpell(GetAreaOfEffectCreator(), oTarget))
        {
            ...
            if(!MySavingThrow(SAVING_THROW_FORT, oTarget, MothFogDC(), SAVING_THROW_ALL, GetAreaOfEffectCreator()))
            {
...
void DoCamoflage(object oTarget)
{
    ...
    if(GetSpellId()==599)//if Harper Spell
    {
     nDuration = GetLevelByClass(CLASS_TYPE_HARPER,OBJECT_SELF);
    }
    nDuration = 10 * nDuration;
...
void DoSpikeGrowthEffect(object oTarget)
{
    ...
    if (spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, oCaster))
    {
        ...
        {
            int nMetaMagic = GetMetaMagicFeat();
            int nDam = MaximizeOrEmpower(8, 4, nMetaMagic);

            effect eDam = EffectDamage(nDam, DAMAGE_TYPE_PIERCING,DAMAGE_POWER_PLUS_SIX);
           ...
           if (GetHasSpellEffect(SPELL_SPIKE_GROWTH, oTarget) == FALSE)
           {
                ...
                if(!MySavingThrow(SAVING_THROW_REFLEX, oTarget, MothFogDC(), SAVING_THROW_ALL,oCaster, fDelay))
                {
                    effect eSpeed = EffectAbilityDecrease(ABILITY_STRENGTH,4);
                    ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eSpeed, oTarget, RoundsToSeconds(nDuration/2));
                }
           }
        }
    }
    ...
    else if (nTouch >0 )
    {
        if(!GetIsReactionTypeFriendly(oTarget))
        {
            ...
            SignalEvent(oTarget, EventSpellCastAt(OBJECT_SELF, nSpellID));
                ...
                if(MySavingThrow(SAVING_THROW_WILL, oTarget, MothDC(), SAVING_THROW_ALL,OBJECT_SELF))
                {
                ...
                if (!MyResistSpell(OBJECT_SELF, oTarget, fDelay))
                {
                    for (i=1; i <= nExtraMissiles + nRemainder; i++)
                    {
                        ...
                        if(nReflexSave)
                        {
                            ...
                            {
                                nDam = MothGetReflexAdjustedDamageTrap(nDam, oTarget, MothDC(), SAVING_THROW_TYPE_ELECTRICITY);
                            }
                            else if ( nDAMAGETYPE == DAMAGE_TYPE_FIRE )
                            {
                                nDam = MothGetReflexAdjustedDamageTrap(nDam, oTarget, MothDC(), SAVING_THROW_TYPE_FIRE);
                            }
                            else if ( nDAMAGETYPE == DAMAGE_TYPE_ACID )
                            {
                                nDam = MothGetReflexAdjustedDamageTrap(nDam, oTarget, MothDC(), SAVING_THROW_TYPE_ACID);
                            }
                        }
...
//    int nDamage = 0;
    ...
    while (GetIsObjectValid(oTarget) && nCnt <= nEnemies)
    {
        ...
        if (spellsIsTarget(oTarget, SPELL_TARGET_SELECTIVEHOSTILE, OBJECT_SELF) && (oTarget != OBJECT_SELF) && (GetObjectSeen(oTarget,OBJECT_SELF)))
        {
                ...
                // * recalculate appropriate distances
                fDist = GetDistanceBetween(OBJECT_SELF, oTarget);
                fDelay = fDist/(4.0 * log(fDist) + 2.0);
                // Firebrand.
                // It means that once the target has taken damage this round from the
                // spell it won't take subsequent damage
                if (nONEHIT == TRUE)
                ...
                if (!MyResistSpell(OBJECT_SELF, oTarget, fDelay))
                {
                    for (i=1; i <= nExtraMissiles + nRemainder; i++)
                    {
                        ...
                        ApplyEffectToObject(DURATION_TYPE_INSTANT, eMissile, oTarget);
                        DelayCommand(fDelay2, ApplyEffectToObject(DURATION_TYPE_INSTANT, eDam, oTarget));
                    }
                } // for
                else
                {  // * apply a dummy visual effect
...
void DoMagicFang(int nPower, int nDamagePower)
{
    ...
    int nDuration = MothGetCasterLevel(OBJECT_SELF); // * Duration 1 turn/level
    ...
    if(!GetIsReactionTypeFriendly(oTarget))
    {
        ...
        switch (nGameDiff)
        {
            ...
            case GAME_DIFFICULTY_NORMAL:
                    fDifficulty = RoundsToSeconds(nPower/2); // One Round per hit-die or caster level
                break;
    ...
    while(GetIsObjectValid(oTarget))
    {
        if (spellsIsTarget(oTarget, nTargetType, oCaster) == TRUE)
        {
          ...
          if(!nResistSpellSuccess)
          {
                ...
                {
                  nSavingThrowSuccess = MySavingThrow(nSavingThrowType, oTarget, MothDC(), nSavingThrowSubType);
                }
...
void spellApplyMindBlank(object oTarget, int nSpellId, float fDelay=0.0)
{
    ...
    int bValid;
    int nDuration = MothGetCasterLevel(OBJECT_SELF);
    if(GetSpellId()==597)//if Harper Spell
    {
     nDuration = GetLevelByClass(CLASS_TYPE_HARPER,OBJECT_SELF);
    }
    int nMetaMagic = GetMetaMagicFeat();
...
//:: Created On:
//:://////////////////////////////////////////////
void doAura2(int nVis1, int nVis2, int nVis3, int nDamageType)
{
    //Declare major variables
    object oTarget = GetSpellTargetObject();
    int nDuration = MothGetCasterLevel(OBJECT_SELF);
    ...
    if (nMetaMagic == METAMAGIC_EXTEND)
    {
       nDuration2 = nDuration2 * 2;    //Duration is +100%
    }
...
void spellsStinkingCloud(object oTarget = OBJECT_INVALID)
{
    ...
    if (GetIsObjectValid(oTarget) == TRUE)
    {
        if(spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE,  GetAreaOfEffectCreator()))
        {
            ...
            SignalEvent(oTarget, EventSpellCastAt(OBJECT_SELF, GetSpellId()));
            //Make a Fort Save
            if(!MySavingThrow(SAVING_THROW_FORT, oTarget, MothFogDC(), SAVING_THROW_TYPE_POISON,GetAreaOfEffectCreator()))
            {
...
void spellsDispelMagic(object oTarget, int nCasterLevel, effect eVis, effect eImpac, int bAll = TRUE, int bBreachSpells = FALSE)
{
    ...
    if (bAll == TRUE )
    {
        ...
        if(GetIsPC(oTarget)){
            ...
            NoahDispelMagicAll(nCasterLevel, oTarget);
        }
        else{
