...
void main()
{
   ...
   { // SPELL/FEAT ID - switch to control horse actions
       ...
       {
           int nCurrent = GetAppearanceType(oTarget);
           int nNext = nCurrent+1;
           if(HorseGetCanBeMounted(oTarget) &&
           GetMaster(oTarget)==oPC &&
           GetLocalInt(oTarget,"bX3_IS_MOUNT")==TRUE
           )
           {
           if(nCurrent==561)
           {
           ...
           SetCreatureAppearanceType(oTarget,nNext);
           SendMessageToPC(oPC,"<c ¥ >*Mounts Appearance Number is now "+IntToString(nNext)+".*</c>");
           }
           else
           {
           SendMessageToPC(oPC,"<c ¥ >*You can only modify your own Mounts!*</c>");
           }
       ...
       { // Assign Mount Menu Selection
           ...
           {
           int nCurrent = GetAppearanceType(oTarget);
           DeleteLocalInt(oPC,"MOTHMOUNTAPP");
           SetLocalInt(oPC,"MOTHMOUNTAPP",nCurrent);
           SendMessageToPC(oPC,"<c ¥ >*Mounts Appearance Number is now stored as "+IntToString(nCurrent)+".*</c>");
           }
           else
           {
           SendMessageToPC(oPC,"<c ¥ >*You can only store the appearance of your own Mounts!*</c>");
           }
