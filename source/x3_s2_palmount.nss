...
void main()
{
    ...
    if(GetHasEffect(EFFECT_TYPE_POLYMORPH,oRider))
       {
        SendMessageToPC(oRider,"<cеее>[Server] You cannot mount while polymorphed.</c>");
        AssignCommand(oRider,HORSE_SupportDismountWrapper(bAnim,TRUE));
        return;
       }
    if(GetLocalInt(oRider,"HasMountExp")== TRUE)
    {
        SendMessageToPC(oRider,"<cеее>[Server] You cannot ride two horses at once!</c>");
        AssignCommand(oRider,HORSE_SupportDismountWrapper(bAnim,TRUE));
        return;
    }
