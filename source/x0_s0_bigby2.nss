...
void RunHandImpact2(object oTarget, object oCaster)
{
    int nBonus2 = MothGetCasterLevel(oCaster)/2;
    if (GZGetDelayedSpellEffectsExpired(460,oTarget,oCaster))
    {
        return;
    }
    int nDam = d12(3);
    ...
    DelayCommand(9.0f,RunHandImpact2(oTarget,oCaster));
}
void main()
{
    ...
    effect eKnockdowna = EffectVisualEffect(VFX_DUR_MIND_AFFECTING_DISABLED);
    effect eKnockdownb = EffectVisualEffect(VFX_DUR_BIGBYS_GRASPING_HAND);
    effect eKnockdown = EffectLinkEffects(eKnockdowna,eKnockdownb);
    effect eKnockdown2 = EffectCutsceneImmobilize();
    ...
    if(!GetIsReactionTypeFriendly(oTarget))
    {
        ...
        if(!MyResistSpell(OBJECT_SELF, oTarget))
        {
            ...
            RunHandImpact2(oTarget, OBJECT_SELF);
            ...
            if (nTargetRoll1==1)
            {
                    ...
                    ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eLink, oTarget, RoundsToSeconds(nDuration * 2 / 3));
                    return;
            }
