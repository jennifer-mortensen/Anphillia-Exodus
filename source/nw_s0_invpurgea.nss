...
void main()
{
    ...
    if (GetHasSpellEffect(SPELL_IMPROVED_INVISIBILITY,oTarget) == TRUE && GetIsReactionTypeHostile(oTarget,OBJECT_SELF) == TRUE)
    {
        RemoveAnySpellEffects(SPELL_IMPROVED_INVISIBILITY,oTarget);
    }
    else if (GetHasSpellEffect(SPELL_IMPROVED_INVISIBILITY,oTarget) == TRUE && GetIsReactionTypeHostile(oTarget,OBJECT_SELF) == TRUE)
    {
        RemoveAnySpellEffects(SPELL_INVISIBILITY, oTarget);
    }
    else if (GetHasSpellEffect(SPELLABILITY_AS_INVISIBILITY,oTarget) == TRUE && GetIsReactionTypeHostile(oTarget,OBJECT_SELF) == TRUE)
    {
        RemoveAnySpellEffects(SPELLABILITY_AS_INVISIBILITY,oTarget);
    }
    else if(GetHasSpellEffect(SPELLABILITY_AS_IMPROVED_INVISIBLITY,oTarget) == TRUE && GetIsReactionTypeHostile(oTarget,OBJECT_SELF) == TRUE)
    {
