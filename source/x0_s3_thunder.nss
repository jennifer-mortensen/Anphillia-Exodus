    ...
    while (GetIsObjectValid(oTarget))
    {
        if(!GetIsReactionTypeFriendly(oTarget))
        {
            ...
            fDelay = GetDistanceBetweenLocations(lTarget, GetLocation(oTarget))/20;
            {
                ...
                if((oTarget != oCaster))
                if(!MySavingThrow(SAVING_THROW_WILL, oTarget, 20))
                {
                    ...
                    DelayCommand(fDelay+0.8, ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eImmune, oTarget, RoundsToSeconds(nDuration)));
                }
             }
        }
