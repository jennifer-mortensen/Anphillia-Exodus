...
void CreateCachedObject(object oSpawn, object oSpawned, int nCacheBucket, int nCachedCondtion = 0);
...
int CountPCsInRadius(location lCenter, float fRadius, int nDM = FALSE) //J. Persinne; revised this function for efficiency.
{
    /*int nPCs = 0;
    ...
    return nPCs;*/
    int i = 1;
    object oPC = GetNearestCreatureToLocation(CREATURE_TYPE_PLAYER_CHAR, PLAYER_CHAR_IS_PC, lCenter, i);
    float fDistance = GetDistanceBetweenLocations(lCenter, GetLocation(oPC));
    while(fDistance <= fRadius && GetIsObjectValid(oPC))
    {
        if(!GetIsDM(oPC))
            return TRUE;
        i++;
        oPC = GetNearestCreatureToLocation(CREATURE_TYPE_PLAYER_CHAR, PLAYER_CHAR_IS_PC, lCenter, i);
        fDistance = GetDistanceBetweenLocations(lCenter, GetLocation(oPC));
    }
    return FALSE;
}
//
...
void DoPatrolRoute(int nPatrolRoute, int nRouteType)
{
    ...
    if (GetCurrentAction() != ACTION_INVALID)
        return;
    if(GetLocalInt(GetArea(OBJECT_SELF), "MY_PC_COUNT") == -1) //J. Persinne; cease patrols if no one is in the area.
        return;
...
//**FLAG          object oSpawned = CreateNessObject(nObjectType, sTemplate, lLastLocation);
   ...
   int nTimeNow, int nWalkToHome = FALSE)
{
    ...
    if (fCorpseDecay > 0.0)
    {
        ...
        if(GetStringLeft(GetResRef(oSpawned), 3) == "ats")
            SetLocalString(oSpawned, "CorpseRemainsResRef", "ats_invis_corpse"); //J. Persinne; ats creatures use an ats corpse.
        else
  ...
  float fHeartbeatInterval = 6.0, float fFirstDelay = 0.0 )
{
  ...
  if ( GetIsPC( oPC ) && !GetIsDM(oPC))
  {
    ...
    DeleteLocalInt(OBJECT_SELF, "HB_COUNT"); //J. Persinne; reset the HB count for this area.
...
//creates a cached object from oSpawned
void CreateCachedObject(object oSpawn, object oSpawned, int nCacheBucket, int nCacheCondition = 0)
{
