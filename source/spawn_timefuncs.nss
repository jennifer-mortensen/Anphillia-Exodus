...
int GetCurrentRealSeconds()
{
    ...
    int nStartYear = GetLocalInt(GetModule(), "ModuleStartYear"); //J. Persinne; use the number of years passed, rather than the start year, with this function
                                                                  //in order to avoid an integer overflow.
    return CalendarToRealSeconds(GetCalendarYear() - nStartYear, GetCalendarMonth(),
