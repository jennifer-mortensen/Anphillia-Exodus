...
void RunHandImpact5(object oTarget, object oCaster)
{
    int nBonus2 = MothGetCasterLevel(oCaster)/2;
    if (GZGetDelayedSpellEffectsExpired(SPELL_BIGBYS_CRUSHING_HAND,oTarget,oCaster))
    {
        return;
    }
    int nDam = d12(6);
    effect eDam = EffectDamage(nDam, DAMAGE_TYPE_BLUDGEONING,DAMAGE_POWER_PLUS_SIX);
    effect eVis = EffectVisualEffect(VFX_IMP_ACID_L);
    ApplyEffectToObject(DURATION_TYPE_INSTANT, eDam, oTarget);
    ApplyEffectToObject(DURATION_TYPE_INSTANT, eVis, oTarget);
    DelayCommand(9.0f,RunHandImpact5(oTarget,oCaster));
}
void main()
{
    ...
    if(!GetIsReactionTypeFriendly(oTarget))
    {
        ...
        if(!MyResistSpell(OBJECT_SELF, oTarget))
        {
            ...
            RunHandImpact5(oTarget, OBJECT_SELF);
            ...
            if (nTargetRoll1==20)
            {
                    SendMessageToPC(OBJECT_SELF,"<cÝ¡Ý>" + GetName(oTarget) + "</c><c ¿þ> : Concentration Save vs. Grappling : *success* :(" + sTargetRoll1 + " + " + sTargetRoll2 + " = " + sTargetRollFinal + " vs DC: " + sCasterRoll + ")</c>");
                    SendMessageToPC(oTarget,"<cðøþ>" + GetName(oTarget) + "</c><c ¿þ> : Concentration Save vs. Grappling : *success* :(" + sTargetRoll1 + " + " + sTargetRoll2 + " = " + sTargetRollFinal + " vs DC: " + sCasterRoll + ")</c>");
                    ApplyEffectToObject(DURATION_TYPE_TEMPORARY,
                                        eHand, oTarget,
                                        RoundsToSeconds(nDuration) * 2 / 3);
                    return;
            ...
            if (nTargetRoll1==1)
                {
                    SendMessageToPC(OBJECT_SELF,"<cÝ¡Ý>" + GetName(oTarget) + "</c><c ¿þ> : Concentration Save vs. Grappling : *faliure* :(" + sTargetRoll1 + " + " + sTargetRoll2 + " = " + sTargetRollFinal + " vs DC: " + sCasterRoll + ")</c>");
                    SendMessageToPC(oTarget,"<cðøþ>" + GetName(oTarget) + "</c><c ¿þ> : Concentration Save vs. Grappling : *faliure* :(" + sTargetRoll1 + " + " + sTargetRoll2 + " = " + sTargetRollFinal + " vs DC: " + sCasterRoll + ")</c>");
                    ApplyEffectToObject(DURATION_TYPE_TEMPORARY,
                                        eLink, oTarget,
                                        RoundsToSeconds(nDuration) * 2 / 3);
                    return;
