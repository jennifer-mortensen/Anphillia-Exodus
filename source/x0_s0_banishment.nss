...
// End of Spell Cast Hook
    ...
    while(GetIsObjectValid(oTarget))
    {
        ...
        if(GetIsReactionTypeFriendly(oTarget2) || GetFactionEqual(oTarget2) || GetIsReactionTypeNeutral(oTarget2))
        {
        effect eBad = GetFirstEffect(oTarget);
        //Search for negative effects
        while(GetIsEffectValid(eBad))
        {
            if (GetEffectType(eBad) == EFFECT_TYPE_SPELL_FAILURE ||
        ...
        if(GetIsObjectValid(oMaster) && spellsIsTarget(oMaster,SPELL_TARGET_SELECTIVEHOSTILE, OBJECT_SELF ))
        {
            ...
            {
                SignalEvent(oTarget, EventSpellCastAt(OBJECT_SELF, SPELL_BANISHMENT));
                //Determine correct save
                nSpellDC = MothDC();
                //Make SR and will save checks
                if (!MyResistSpell(OBJECT_SELF, oTarget) && !MySavingThrow(SAVING_THROW_FORT, oTarget, nSpellDC))
                {
