...
void FXWand_MothMystExplo()
{
  location lTarget = GetLocation( oDM);

  AssignCommand( oDM, DelayCommand( 1.0, ApplyEffectAtLocation ( DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_FNF_MYSTICAL_EXPLOSION), lTargetLoc)));
}
void FXWand_MothElecExplo()
{
  location lTarget = GetLocation( oDM);

  AssignCommand( oDM, DelayCommand( 1.0, ApplyEffectAtLocation ( DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_FNF_ELECTRIC_EXPLOSION), lTargetLoc)));
}
void FXWand_MothGoodAoE()
{
  location lTarget = GetLocation( oDM);

  AssignCommand( oDM, DelayCommand( 1.0, ApplyEffectAtLocation ( DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_FNF_LOS_HOLY_30), lTargetLoc)));
}
void FXWand_MothNeutralAoE()
{
  location lTarget = GetLocation( oDM);

  AssignCommand( oDM, DelayCommand( 1.0, ApplyEffectAtLocation ( DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_FNF_LOS_NORMAL_30), lTargetLoc)));
}
void FXWand_MothEvilAoE()
{
  location lTarget = GetLocation( oDM);

  AssignCommand( oDM, DelayCommand( 1.0, ApplyEffectAtLocation ( DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_FNF_LOS_EVIL_30), lTargetLoc)));
}
void FXWand_MothGasAcid()
{
  location lTarget = GetLocation( oDM);

  AssignCommand( oDM, DelayCommand( 1.0, ApplyEffectAtLocation ( DURATION_TYPE_TEMPORARY, EffectVisualEffect(VFX_FNF_GAS_EXPLOSION_ACID), lTargetLoc, 20.0)));
}
void FXWand_MothGasEvil()
{
  location lTarget = GetLocation( oDM);

  AssignCommand( oDM, DelayCommand( 1.0, ApplyEffectAtLocation ( DURATION_TYPE_TEMPORARY, EffectVisualEffect(VFX_FNF_GAS_EXPLOSION_EVIL), lTargetLoc, 20.0)));
}
void FXWand_MothGasFire()
{
  location lTarget = GetLocation( oDM);

  AssignCommand( oDM, DelayCommand( 1.0, ApplyEffectAtLocation ( DURATION_TYPE_TEMPORARY, EffectVisualEffect(VFX_FNF_GAS_EXPLOSION_FIRE), lTargetLoc, 20.0)));
}
void FXWand_MothGasGrease()
{
  location lTarget = GetLocation( oDM);

  AssignCommand( oDM, DelayCommand( 1.0, ApplyEffectAtLocation ( DURATION_TYPE_TEMPORARY, EffectVisualEffect(VFX_FNF_GAS_EXPLOSION_GREASE), lTargetLoc, 20.0)));
}
void FXWand_MothGasMind()
{
  location lTarget = GetLocation( oDM);

  AssignCommand( oDM, DelayCommand( 1.0, ApplyEffectAtLocation ( DURATION_TYPE_TEMPORARY, EffectVisualEffect(VFX_FNF_GAS_EXPLOSION_MIND), lTargetLoc, 20.0)));
}
void FXWand_MothGasNature()
{
  location lTarget = GetLocation( oDM);

  AssignCommand( oDM, DelayCommand( 1.0, ApplyEffectAtLocation ( DURATION_TYPE_TEMPORARY, EffectVisualEffect(VFX_FNF_GAS_EXPLOSION_NATURE), lTargetLoc, 20.0)));
}
//void main() {}
