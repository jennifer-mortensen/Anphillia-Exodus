...
if (GetItemPossessedBy(oPC, "crystalofcosmica")!= OBJECT_INVALID)
   {
   nInt = GetLocalInt(oMod, "mothstatue1");
   nInt += 1;
   SetLocalInt(oMod, "mothstatue1", nInt);
   DelayCommand(2.0,FloatingTextStringOnCreature("<cýõæ>"+sPC+" inserts the Crystal in the statue. The statue slowly descend into the floor.</c>",oPC));
   ...
   object oItem;
   oItem = GetItemPossessedBy(oPC, "crystalofcosmica");
