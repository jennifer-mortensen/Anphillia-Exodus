...
void main()
{
    ...
    {
        int nDamage = ArcaneArcherDamageDoneByBow()*6;
        if (nDamage > 0)
        {
            effect ePhysical = EffectDamage(nDamage+28, DAMAGE_TYPE_PIERCING,IPGetDamagePowerConstantFromNumber(nBonus));
            effect eMagic = EffectDamage(nAA*3, DAMAGE_TYPE_MAGICAL);
