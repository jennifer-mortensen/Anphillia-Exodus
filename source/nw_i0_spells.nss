...
//::        - Visual duration effects for CreateProtectionFromAlignmentLink taken off.
//:://////////////////////////////////////////////
#include "moth_inc"
#include "_inc_gen"

// GZ: Number of spells in GetSpellBreachProtections
const int NW_I0_SPELLS_MAX_BREACH = 37;
...
void spellsCure(int nDamage, int nMaxExtraDamage, int nMaximized, int vfx_impactHurt, int vfx_impactHeal, int nSpellID)
{
    ...
    int nExtraDamage = MothGetCasterLevel(OBJECT_SELF); // * figure out the bonus damage
...
{
  int n = MothGetCasterLevel(OBJECT_SELF);
...
int MyResistSpell(object oCaster, object oTarget, float fDelay = 0.0)
{
    ...
    if(GetLocalInt(oCaster,"MOTHISPMCASTER")==TRUE)
    {
            if(nResist == 1 && MothIsSpellRealNecromancySpell(GetSpellId())==FALSE) //Spell Resistance
            {
            ...
            else if(nResist == 1 && MothIsSpellRealNecromancySpell(GetSpellId())==TRUE) //Spell Resistance and caster is a PM
            {
              return FALSE;
            }

    }
    else if(GetIsPC(oCaster) && !GetIsDM(oCaster) && GetAppearanceType(oCaster) > 6) //Handling for shifted characters that ignore spell resistance.
    {
            if(nResist == 2) //Globe
            {
                DelayCommand(fDelay, ApplyEffectToObject(DURATION_TYPE_INSTANT, eGlobe, oTarget));
            ...
            else if(nResist == 1) //Spell Resistance
            {
                if(nSpellId == 36) //Darkness
                {
                    if(GetIsPolymorphType(oCaster, ANPH_POLYMORPH_TYPE_DRIDER_MALE) ||
                ...
                else if(nSpellId == 132) //Power Word: Stun
                {
                    if(GetIsPolymorphType(oCaster, ANPH_POLYMORPH_TYPE_MINDFLAYER_EPIC))
                        return FALSE;
                }
                else if(nSpellId == 75) //Gust of Wind
                {
                    if(GetIsPolymorphType(oCaster, ANPH_POLYMORPH_TYPE_MANTICORE_EPIC))
                        return FALSE;
                }
                else if(nSpellId == 188) //Vampiric Touch
                {
                    if(GetIsPolymorphType(oCaster, ANPH_POLYMORPH_TYPE_VAMPIRE_MALE) ||
                        GetIsPolymorphType(oCaster, ANPH_POLYMORPH_TYPE_VAMPIRE_FEMALE))
                            return FALSE;
                }
                else
                {
    ...
    else
    {
            ...
            else if(nResist == 1 && nSpellId == 832) //Spell Resistance and the spell is a special subrace spell (in this case darkness).
            {
                return FALSE;
            }
            else if(nResist == 1) //Spell Resistance
...
//:: Modified By: Rabidness
//:: Modified On: June 20, 2004
//::        - Visual duration effects taken off, yes... even Cessate_positive.
//:://////////////////////////////////////////////
effect CreateProtectionFromAlignmentLink(int nAlignment, int nPower = 1)
{
    int nFinal = nPower * 2;
...
int GetSpellBreachProtection(int nLastChecked)
{
    ...
    //--------------------------------------------------------------------------
    if(nLastChecked == 1) {return 527;}//Stone Bones
    else if (nLastChecked == 2){return SPELL_PREMONITION;}
    else if(nLastChecked == 3) {return SPELL_SPELL_MANTLE;}
    else if(nLastChecked == 4) {return 382;} //Protection Domain Sanctuary
    else if(nLastChecked == 5) {return SPELL_SHADOW_SHIELD;}
    else if(nLastChecked == 6) {return SPELL_STONESKIN;}
    else if(nLastChecked == 7) {return SPELL_ETHEREAL_VISAGE;}
    else if(nLastChecked == 8) {return SPELL_GLOBE_OF_INVULNERABILITY;}
    else if(nLastChecked == 9) {return 12;}//Moth: Calm Emotions
    else if(nLastChecked == 10) {return SPELL_PROTECTION__FROM_CHAOS;} //Moth: Greater Sanctuary replaced by Protection from Chaos
    else if(nLastChecked == 11) {return SPELL_MINOR_GLOBE_OF_INVULNERABILITY;}
    else if(nLastChecked == 12) {return SPELL_SPELL_RESISTANCE;}
    else if(nLastChecked == 13) {return SPELL_GREATER_STONESKIN;}
    else if(nLastChecked == 14) {return SPELL_LESSER_SPELL_MANTLE;}
    else if(nLastChecked == 15) {return SPELL_MESTILS_ACID_SHEATH;}
    else if(nLastChecked == 16) {return SPELL_LESSER_MIND_BLANK;}
    else if(nLastChecked == 17) {return SPELL_ELEMENTAL_SHIELD;}
    else if(nLastChecked == 18) {return SPELL_PROTECTION_FROM_SPELLS;}
    else if(nLastChecked == 19) {return SPELL_PROTECTION_FROM_ELEMENTS;}
    else if(nLastChecked == 20) {return SPELL_RESIST_ELEMENTS;}
    else if(nLastChecked == 21) {return SPELL_DEATH_ARMOR;}
    else if(nLastChecked == 22) {return SPELL_GHOSTLY_VISAGE;}
    else if(nLastChecked == 23) {return SPELL_ENDURE_ELEMENTS;}
    else if(nLastChecked == 24) {return 162;}//Moth: Replaced a second(!?) Shadow Shield check with Shield of Law
    else if(nLastChecked == 25) {return SPELL_SHADOW_CONJURATION_MAGE_ARMOR;}
    else if(nLastChecked == 26) {return SPELL_NEGATIVE_ENERGY_PROTECTION;}
    else if(nLastChecked == 27) {return SPELL_SANCTUARY;}
    else if(nLastChecked == 28) {return SPELL_MAGE_ARMOR;}
    else if(nLastChecked == 29) {return SPELL_STONE_BONES;}
    else if(nLastChecked == 30) {return SPELL_SHIELD;}
    else if(nLastChecked == 31) {return SPELL_SHIELD_OF_FAITH;}
    else if(nLastChecked == 32) {return SPELL_MIND_BLANK;}
    else if(nLastChecked == 33) {return SPELL_IRONGUTS;}
    else if(nLastChecked == 34) {return SPELL_ENERGY_BUFFER;}
    else if(nLastChecked == 35) {return SPELL_RESISTANCE;}
    else if(nLastChecked == 36) {return 85;}//Golem Walk
    else if(nLastChecked == 37) {return SPELL_GREATER_SPELL_MANTLE;}
    else if(nLastChecked == 38) {return 330;}//Chaos shield
    return nLastChecked;
}
