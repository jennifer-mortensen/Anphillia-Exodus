...
void  AddBonusToThrowing(object oMyArmor, float fDuration, int nAmount)
{
...
void main()
{
    ...
    {
        if (GetBaseItemType(oShield) ==63 ||
            GetBaseItemType(oShield) ==59 ||
            GetBaseItemType(oShield) ==31
        )
        {
            nValid = TRUE;
        }
    ...
    if(oTarget==OBJECT_SELF || oTarget==oShield && nValid == TRUE)
    {
        ...
        if (nDuration>0)
        {
            ...
            AddBonusToThrowing(oShield, TurnsToSeconds(nDuration),nAmount);
        }
        return;
    }
    else
