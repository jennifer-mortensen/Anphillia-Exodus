...
void main()
{
    ...
    effect eLightning = EffectBeam(VFX_BEAM_FIRE_W, OBJECT_SELF, BODY_NODE_HAND);;
    ...
    {
        SignalEvent(oFirstTarget, EventSpellCastAt(OBJECT_SELF, 331));
        if (!MyResistSpell(OBJECT_SELF, oFirstTarget))
        {
            nDamStrike = MothGetReflexAdjustedDamage(nDamage, oFirstTarget, MothDC(), SAVING_THROW_TYPE_FIRE);
    ...
    eLightning = EffectBeam(VFX_BEAM_FIRE_W, oFirstTarget, BODY_NODE_CHEST);
    float fDelay = 0.2;
    int nCnt = 0;
    ...
    while (GetIsObjectValid(oTarget) && nCnt < nCasterLevel)
    {
        if (oTarget != oFirstTarget && spellsIsTarget(oTarget, SPELL_TARGET_SELECTIVEHOSTILE, OBJECT_SELF) && oTarget != OBJECT_SELF)
        {
            ...
            SignalEvent(oTarget, EventSpellCastAt(OBJECT_SELF, 331));
            if (!MyResistSpell(OBJECT_SELF, oTarget, fDelay))
            {
                ...
                nDamStrike = MothGetReflexAdjustedDamage(nDamage, oTarget, MothDC(), SAVING_THROW_TYPE_FIRE);
                eDamage = EffectDamage(nDamStrike /2, DAMAGE_TYPE_FIRE);
                if(nDamStrike > 0)
                {
            ...
            {
            eLightning = EffectBeam(VFX_BEAM_FIRE_W, oHolder, BODY_NODE_CHEST);
            }
            else
            {
                effect eNewLightning = EffectBeam(VFX_BEAM_FIRE_W, oHolder, BODY_NODE_CHEST);
