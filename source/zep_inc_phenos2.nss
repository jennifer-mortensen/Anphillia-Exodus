...
const float fSPEED_AURENTHIL = 1.9;
const float fSPEED_HORSE_BLACK = 1.9;
const float fSPEED_HORSE_BROWN = 1.9;
const float fSPEED_HORSE_NIGHTMARE = 1.9;
const float fSPEED_HORSE_WHITE = 1.9;
const float fSPEED_PONY_BROWN = 1.86;
const float fSPEED_PONY_LTBROWN = 1.86;
const float fSPEED_PONY_SPOT = 1.86;
...
effect eDisApp = EffectDisappearAppear(GetLocation(oMount));
    if (oMount !=OBJECT_INVALID)
    {
       ...
       DelayCommand(2.0, DestroyObject(oMount));
    }
// Speed Adjustment & Give Item
 if(GetLevelByClass(CLASS_TYPE_MONK,oRider) <=3)
      {
...
//Restore Mount
DelayCommand(3.0,SetLocalInt(oRider,"HasMountExp",FALSE));
