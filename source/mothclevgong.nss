...
void AnphSendWarningCall5 (object oPC)
{
    string sPCTeam;
    string sTeam = AnphGetPlayerTeam (oPC);
    object oArea = GetArea (oPC);
    AnphSendMessageToTeam (sTeam, "<c  ¥>[Player] "+GetName(oPC)+" is calling for a Cleaven meeting in "
                           +GetName(oArea)+"!</c>");
...
void main()
{
  ...
  AnphSendWarningCall5(GetLastUsedBy());
}
