...
const int HORSE_DEFAULT_SPEED_INCREASE= 10;
...
{ // PURPOSE: Change the movement speed based on that of the horse (wont work unless EffectMovementSpeedIncrease() bug is fixed)
    ...
    int nPaladin = GetLevelByClass(CLASS_TYPE_PALADIN,oRider);
    int nPDK = GetLevelByClass(CLASS_TYPE_PURPLE_DRAGON_KNIGHT,oRider);
    int nLevel  = nPaladin+nPDK;
    int nAttackBonus = 0;
    int nDiff = GetSkillRank(SKILL_RIDE,oRider,TRUE)/9;
    if(GetHasFeat(FEAT_MOUNTED_COMBAT,oRider))
    {
      nAttackBonus = nLevel/15;
      if(nAttackBonus<1)
      {
      nAttackBonus==0;
      }
    ...
    SetLocalInt(oRider,"nX3_SpeedIncrease",nSpeed);
    if (GetLocalInt(oRider,"mothgenblocksummon"))
    {
        nSpeed=8;
        if (nMonk>2)
        {
        eMovement = SupernaturalEffect(EffectVisualEffect(VFX_DUR_CESSATE_NEUTRAL));
        }
        else
        {
    ...
    eEffect   = SupernaturalEffect(EffectACIncrease(1+nDiff,AC_DODGE_BONUS));
    eEffect2  = SupernaturalEffect(EffectAttackIncrease(1+nAttackBonus));
    ApplyEffectToObject(DURATION_TYPE_PERMANENT,eEffect,oRider);
    ApplyEffectToObject(DURATION_TYPE_PERMANENT,eEffect2,oRider);
    ApplyEffectToObject(DURATION_TYPE_PERMANENT,eMovement,oRider);
    }
...
{ // PURPOSE: Remove speed increases based upon mounted
    ...
    eSearch=GetFirstEffect(oRider);
    while(GetIsEffectValid(eSearch))
    { // cycle through effects
        if((GetEffectType(eSearch) == EFFECT_TYPE_AC_INCREASE)&&
          (GetEffectDurationType(eSearch) == DURATION_TYPE_PERMANENT) &&
          (GetEffectSubType(eSearch) == SUBTYPE_SUPERNATURAL))
        { // check to see if matches conditions
            RemoveEffect(oRider,eSearch);
    ...
    while(GetIsEffectValid(eSearch))
    { // cycle through effects
        if((GetEffectType(eSearch) == EFFECT_TYPE_ATTACK_INCREASE)&&
          (GetEffectDurationType(eSearch) == DURATION_TYPE_PERMANENT) &&
          (GetEffectSubType(eSearch) == SUBTYPE_SUPERNATURAL))
        { // check to see if matches conditions
            RemoveEffect(oRider,eSearch);
...
{ // PURPOSE: Check for feats and adjust penalties to archery while mounted
    ...
    { // Rider is mounted and has a ranged weapon - apply penalty
        ...
        { // remove existing penalties before applying the new one
            ...
            { // traverse effects
               if ((GetEffectType(eEffect)==EFFECT_TYPE_ATTACK_INCREASE)&&
        ...
        ApplyEffectToObject(DURATION_TYPE_PERMANENT,SupernaturalEffect(EffectAttackIncrease(nNewPenalty)),oRider);
...
{ // PURPOSE: Apply AC bonus
    ...
    effect eEffect2;
    effect eEffect;
    int nLevel1 = GetLevelByClass(CLASS_TYPE_PALADIN,oRider);
    int nLevel2 = GetLevelByClass(CLASS_TYPE_PURPLE_DRAGON_KNIGHT,oRider);
    int nLevel  = nLevel1+nLevel2;
    int nAttackBase = 1;
    int nAttackBonus = 0;
    int nAttack = 0;
    int nDiff = GetSkillRank(SKILL_RIDE,oRider,TRUE)/8;
    if(nDiff<1){nDiff==1;}
    if(GetHasFeat(FEAT_MOUNTED_COMBAT,oRider))
    {
      nAttackBonus = nLevel/10;
      if(nAttackBonus<0){nAttackBonus==0;}
      nAttack = nAttackBase+nAttackBonus;
    }
    eEffect =EffectACIncrease(nDiff,AC_DODGE_BONUS);
    eEffect2=EffectAttackIncrease(nAttack);
    eEffect =EffectLinkEffects(eEffect1,eEffect2);
    ...
    { // resref specified
        ...
        { // horse successfully created
            ...
            { // assign owner
                ...
                if (GetMaster(oHorse)==oOwner) SetName(oHorse,GetName(oOwner)+"'s Mount");
...
{ // PURPOSE: Summon Paladin Mount
    ...
    object oMount;
    location lLoc;
    int nLevel=GetHitDice(oSummoner)+5;
    int nDespawnTime;
    int nCurrentTime;
    int nMountNum=1;
