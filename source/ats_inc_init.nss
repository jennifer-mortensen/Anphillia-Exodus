...
{
    //No tradeskill journal?  give them one
    if(!GetIsObjectValid(GetItemPossessedBy(oPlayer, CSTR_TRADESKILL_JOURNAL)))
        DelayCommand(1.0, ATS_CreateItemOnPlayer(ATS_GetResRefFromTag(CSTR_TRADESKILL_JOURNAL), oPlayer));

    //Tradeskill box? load their skills in, delete box
    object oBox = GetItemPossessedBy(oPlayer,CSTR_PERSISTENT_SKILLS_BOXTAG);
    if(GetIsObjectValid(oBox)){
        DelayCommand(1.0, ATS_UpdatePlayerSkillTokens(oPlayer));
/*        oBox = GetItemPossessedBy(oPlayer,CSTR_PERSISTENT_SKILLS_BOXTAG);
        while(GetIsObjectValid(oBox)){
            DelayCommand(5.0, DestroyObject(oBox));
            oBox = GetItemPossessedBy(oPlayer,CSTR_PERSISTENT_SKILLS_BOXTAG);
        }*/
    }
...
void ATS_Initialize()
{
    ...
    AssignCommand(GetModule(), DelayCommand(2.0, ATS_LoadRecipes()));
    DelayCommand(5.0, ExecuteScript("ats_init_forge", GetModule()));
    DelayCommand(8.0, ExecuteScript("ats_init_msp", GetModule()));
