...
void main()
{
   if (!GetHasFeat(FEAT_BARD_SONGS, OBJECT_SELF))
   {
    ...
    int nHarper = GetLevelByClass(CLASS_TYPE_HARPER);
    int nHaperBonus = 0;
    if(nHarper>4)
    {
     nHaperBonus = nHarper-1;
    }
    int nLevel = GetLevelByClass(CLASS_TYPE_BARD);
    nLevel = nLevel+nHaperBonus;
    ...
    while(GetIsObjectValid(oTarget))
    {
        if(spellsIsTarget(oTarget, SPELL_TARGET_SELECTIVEHOSTILE, OBJECT_SELF))
        {
            ...
            {
                if(!GetHasFeatEffect(871, oTarget)&& !GetHasSpellEffect(GetSpellId(),oTarget) && !GetHasSpellEffect(823, oTarget))
                {
                    if (nHP > 0)
                    {
