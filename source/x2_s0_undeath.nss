        ...
        if (spellsIsTarget(oTarget, SPELL_TARGET_SELECTIVEHOSTILE, OBJECT_SELF) && oTarget != OBJECT_SELF)
        {
            ...
            if (!MyResistSpell(OBJECT_SELF, oTarget, fDelay))
            {
                if(GetRacialType(oTarget) == RACIAL_TYPE_UNDEAD)
                {
                    ...
                    {
                      if(MySavingThrow(SAVING_THROW_WILL, oTarget, MothDC(), SAVING_THROW_TYPE_POSITIVE, OBJECT_SELF, fDelay))
                        {
                            nDamage = nDamage/2;
                        }
                    }
                    eDam = EffectDamage(nDamage, DAMAGE_TYPE_POSITIVE,DAMAGE_POWER_ENERGY);
